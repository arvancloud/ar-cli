...
// NewCmdDnsCreateCName returns new cobra command to create DNS CName record
func NewCmdDnsCreateCName(in io.Reader, out, errout io.Writer) *cobra.Command {
	var hostFlag string
	...
	cmd := &cobra.Command{
		Use:   "cname domain",
		Short: "create CName record DNS for domain name (Example: example.com)",
		// TODO
		Long: heredoc.Doc(`
