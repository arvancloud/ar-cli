...
// NewCmdDnsUpdateDKIM returns new cobra command to update DNS DKIM record
func NewCmdDnsUpdateDKIM(in io.Reader, out, errout io.Writer) *cobra.Command {
	var textFlag string
	var nameFlag string
	var ttlFlag int
	var cloudFlag bool
	...
	cmd := &cobra.Command{
		Use:   "dkim domain id",
		Short: "update DKIM record DNS for domain name (Example: example.com 1)",
		// TODO
		Long: heredoc.Doc(`
