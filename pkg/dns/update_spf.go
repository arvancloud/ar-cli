...
// NewCmdDnsUpdateSPF returns new cobra command to update DNS SPF record
func NewCmdDnsUpdateSPF(in io.Reader, out, errout io.Writer) *cobra.Command {
	var textFlag string
	var nameFlag string
	var ttlFlag int
	var cloudFlag bool
	...
	cmd := &cobra.Command{
		Use:   "spf domain id",
		Short: "update SPF record DNS for domain name (Example: example.com 1)",
		// TODO
		Long: heredoc.Doc(`
    ...
    establish an authenticated session, and save connection to the configuration file.`),
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(textFlag) > 0 {
				spfRecord := models.SpfRecord{
					Text: textFlag,
				}
				record.Value = &models.OneOfDnsRecordValue{SpfRecord: spfRecord}
			}
