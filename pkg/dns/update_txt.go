...
// NewCmdDnsUpdateTXT returns new cobra command to update DNS TXT record
func NewCmdDnsUpdateTXT(in io.Reader, out, errout io.Writer) *cobra.Command {
	var textFlag string
	var nameFlag string
	var ttlFlag int
	var cloudFlag bool
	...
	cmd := &cobra.Command{
		Use:   "txt domain id",
		Short: "update TXT record #id for domain name (Example: example.com 1)",
		// TODO
		Long: heredoc.Doc(`
    ...
    establish an authenticated session, and save connection to the configuration file.`),
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if len(textFlag) > 0 {
				...
				record.Value = &models.OneOfDnsRecordValue{TxtRecord: txtRecord}
			}
