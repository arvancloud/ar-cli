...
// NewCmdDnsCreateNS returns new cobra command to create DNS NS record
func NewCmdDnsCreateNS(in io.Reader, out, errout io.Writer) *cobra.Command {
	var hostFlag string
	var nameFlag string
	var ttlFlag int
	var cloudFlag bool
	...
	cmd := &cobra.Command{
		Use:   "ns domain",
		Short: "create NS record DNS for domain name (Example: example.com)",
		// TODO
		Long: heredoc.Doc(`
