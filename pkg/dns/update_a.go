...
// NewCmdDnsUpdateA returns new cobra command to update DNS A record
func NewCmdDnsUpdateA(in io.Reader, out, errout io.Writer) *cobra.Command {
	var ipFlag string
	var portFlag int
	var weightFlag int
	...
	cmd := &cobra.Command{
		Use:   "a domain id",
		Short: "update A record #id for domain name (Example: example.com 1)",
		// TODO
		Long: heredoc.Doc(`
