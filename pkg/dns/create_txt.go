...
// NewCmdDnsCreateTXT returns new cobra command to create DNS TXT record
func NewCmdDnsCreateTXT(in io.Reader, out, errout io.Writer) *cobra.Command {
	var textFlag string
	var nameFlag string
	var ttlFlag int
	var cloudFlag bool
	...
	cmd := &cobra.Command{
		Use:   "txt domain",
		Short: "create TXT record DNS for domain name (Example: example.com)",
		// TODO
		Long: heredoc.Doc(`
